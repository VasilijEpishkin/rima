...
primary_tool: MiXCR
workflow: true
source: https://github.com/GPTomics/bioSkills/tree/main/workflows/tcr-pipeline
note_for_this_project: MiXCR-specific stages (activation/license, presets, mixcr analyze) are NOT used here — MiXCR is deliberately excluded from the rima BCR benchmark project. The BCR-specific Immcantation fork (Stage 3b) still applies regardless of which tool (TRUST4/IgReC) produced the input AIRR TSV.
depends_on:
