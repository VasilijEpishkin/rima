#!/usr/bin/env bash
# primer_trim.sh — только V-праймерный тримминг (MaskPrimers + fastp).
#
# Для human (PRJEB40348) и horse (PRJNA848968). Macaque/sheep = 5'RACE, не нужны.
#
# Pipeline для КАЖДОЙ пары:
#   1. MaskPrimers.py align — local alignment, режет 5'-V-праймер
#      (mismatch 0.2, maxlen 50).
#   2. fastp — фильтрация (Q30, minlen 200) + QC-отчёт
#
# FastQC/MultiQC — отдельный шаг (qc_raw.sh --label pr_trimmed).
#
...
#
# Зависимости: pRESTO (MaskPrimers.py), fastp
set -u
...
export SRC PRIMERS PR TMP NPROC
