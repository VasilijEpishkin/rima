#!/usr/bin/env bash
# adapter_trim.sh — cutadapt (Illumina адаптер) + fastp (Q30, minlen, QC).
#
# Pipeline: cutadapt (точное удаление TruSeq/Nextera адаптера по всей длине рида)
#           → fastp (авто-детект остатков, Q30, minlen 250, 8 потоков)
#           → FastQC → MultiQC (только на trimmed)
#
# Запуск:
...
#
# Результаты:    results/<DS>/trimmed/{fastp_reports, fastqc, multiqc, fastq}/
#
# Зависимости: cutadapt, fastp, fastqc, multiqc (python3 -m multiqc)
set -euo pipefail
shopt -s nullglob
...
FQC=$BASE/fastqc                     # FastQC на trimmed
...
# --- 3. FastQC на trimmed ---
echo "[$DS] FastQC ..."
fastqc -t 8 -o "$FQC" "$TRIM"/*.trim.fastq.gz 2>/dev/null
echo "[$DS] FastQC done: $(ls "$FQC"/*_fastqc.html 2>/dev/null | wc -l) reports"

# --- 4. MultiQC на trimmed ---
echo "[$DS] MultiQC ..."
python3 -m multiqc "$FQC" -o "$MQC" -n "${DS}_trimmed_multiqc" -f 2>&1
echo "[$DS] MultiQC done: $(ls "$MQC"/*.html 2>/dev/null)"
